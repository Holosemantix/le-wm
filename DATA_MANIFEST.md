...
## Current three-seed PLDM diagnostic sweep (2026-07-23)

The current manuscript extends the PLDM sweep to three independently trained checkpoint families while keeping the metric implementation unchanged. The PLDM experiment reports how planning success, relative IR, and SR vary across the Gaussian-noise training sweep. It does not select PLDM screening thresholds or evaluate PLDM classification accuracy.
...
| `assets/paper1_figs/fig_pldm_sweep_diagnostics.pdf` | Three-run PLDM sweep figure | `3b9079a5cfc5081eaaa6a94cf2b4043d48bec2bf77b6df74549d3f069a13bf31` |
...
- **Archived E2 PLDM architecture portability**: `paper1/tables/table_pldm_architecture_portability.tex` preserves the immutable public-v1 result for one independently trained four-task PLDM checkpoint family (36 rows). This historical threshold-classification audit is not part of the current manuscript.
...
- The current PLDM Gaussian sweep combines that source with the seed-3073/3074 manifests and frozen diagnostic artifacts listed above. It uses the same condition names, success-rate units, and evaluation seeds as the LeWM sweep and is reported as diagnostic behavior rather than threshold classification.
