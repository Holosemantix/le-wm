...
- **E2 PLDM architecture portability**: `paper1/tables/table_pldm_architecture_portability.tex` summarizes one independently trained four-task PLDM checkpoint family (36 rows) under the current task-relative protocol. PLDM-local and task-relative LeWM-source calibration both give BA/precision/recall `0.836/0.789/0.882` on this grid. This numerical agreement does not make the raw calibration architecture-invariant. Evaluation seeds 42/43/44 are conditional measurement replicates, not PLDM training seeds.
