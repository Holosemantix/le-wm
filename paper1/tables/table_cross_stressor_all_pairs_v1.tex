\begin{table*}[t]
\centering
\caption{All 24 LeWM checkpoint pairs evaluated under blur and resize. ATR$_{\rm rel}$ and SMPR are measured for the checkpoint trained with Gaussian augmentation ($\stdmax{}=0.08$); $\Delta P$ and $\Delta S$ are its success-rate and selective-score changes relative to the unaugmented checkpoint. A pair is positive when $\Delta P\geq5$ percentage points with at most a five-point clean loss. Daggers mark the two discordant pairs.}
\label{tab:cross-stressor-all-pairs}
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{lrlrrrrrl}
\toprule
Task & Seed & Shift & ATR$_{\rm rel}$ & SMPR & $\Delta P$ & $\Delta S$ & Outcome (success / score) \\
\midrule
