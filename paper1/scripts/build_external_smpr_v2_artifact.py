#!/usr/bin/env python3
"""Bind held-out SMPR v2 task shards to the immutable diagnostic protocol."""
...
from paper1.scripts.build_smpr_v2_artifact import (
    ROOT,
    TASKS,
    build_artifact,
)
...
def build_external_artifact(
    *,
    ...
    _require(training_seed in heldout_seeds, "SMPR seed is not frozen held-out TEST data")
    ...
    _require(reference_meta.get("training_seed") == training_seed, "ATR seed mismatch")
    _require(reference_meta.get("training_family_id") == family_id, "ATR family mismatch")
    _require(reference_meta.get("split_name") == "TEST", "ATR is not held-out TEST data")
    ...
    for key, expected in expected_protocol.items():
        _require(smpr_protocol.get(key) == expected, f"SMPR protocol field mismatch: {key}")
    ...
    base_builder_path = ROOT / "paper1" / "scripts" / "build_smpr_v2_artifact.py"
    ...
    metadata.update(
        {
            ...
            "split_name": "TEST",
            "artifact_role": "heldout_external_smpr",
            "behavior_blind": True,
    ...
    _require(len(diagnostic_rows) == len(TASKS) * 9, "held-out SMPR coverage mismatch")
    return artifact
