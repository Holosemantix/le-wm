#!/usr/bin/env python3
"""Compare IR--SR score changes with blur/resize planning changes."""
...
DEFAULT_P2 = ROOT / "paper1/results/cross_task_ir_sr_all_subsets_summary_v2.json"
DEFAULT_ROWS = ROOT / "paper1/results/external_validation/cross_stressor_three_source_ir_sr_v2.csv"
DEFAULT_SUMMARY = ROOT / "paper1/results/external_validation/cross_stressor_three_source_ir_sr_v2.json"
DEFAULT_TABLE = ROOT / "paper1/tables/table_cross_stressor_ir_sr_summary_v2.tex"
DEFAULT_ALL_PAIRS = ROOT / "paper1/tables/table_cross_stressor_ir_sr_all_pairs_v2.tex"
DEFAULT_FIGURE = ROOT / "assets/paper1_figs/fig_cross_stressor_ir_sr_comparison_v2.pdf"
...
def _score(
    ...
    separation_margin = (sr - sr_threshold) / (abs(sr_threshold) + 1e-12)
...
def _metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "recall": recall,
        "f1": f1,
        "spearman_delta_behavior_vs_delta_ir_sr_score": _spearman(
            [row["delta_behavior"] for row in rows],
            [row["delta_ir_sr_score"] for row in rows],
        ),
        "signed_order_agreement": sum(
            (row["delta_behavior"] > 0) == (row["delta_ir_sr_score"] > 0)
            for row in rows
        )
        / len(rows),
...
def build(
    ...
    pair_rows = to_ir_sr(pair_rows)
    p2_summary = to_ir_sr(p2_summary)
    ...
    for row in lewm:
        task = row["task"]
        ir_threshold, sr_threshold = thresholds[task]
        ...
        base_sr = float(row["base_sr"])
        endpoint_sr = float(row["endpoint_sr"])
        base_score = _score(
            base_ir_relative,
            base_sr,
            ir_threshold,
        ...
        output.append(
            {
                ...
                "base_ir_sr_score": base_score,
                "endpoint_ir_sr_score": endpoint_score,
                "delta_ir_sr_score": delta_score,
...
def write_summary_table(summary: dict[str, Any], out: Path) -> None:
    ...
    for label, metrics in rows:
        lines.append(
            ...
            f"{metrics['spearman_delta_behavior_vs_delta_ir_sr_score']:.3f} & "
...
def write_all_pairs_table(rows: list[dict[str, Any]], out: Path) -> None:
    ...
    for row in rows:
        lines.append(
            ...
            f"{row['endpoint_ir_relative']:.3f} & {row['endpoint_sr']:.3f} & "
            f"{row['delta_behavior']:.1f} & {row['delta_ir_sr_score']:.3f} & "
...
def plot(rows: list[dict[str, Any]], out: Path) -> None:
    ...
    and the IR--SR score change (bottom). No thresholds or verdicts are
    ...
    with plt.rc_context(STYLE):
        ...
        for task_index, task in enumerate(TASKS):
            ...
            for offset, stressor in enumerate(stressors):
                ...
                score_means.append(
                    mean(row["delta_ir_sr_score"] for row in cell)
                )
        ...
        score_ax.set_ylabel("Joint screen score\nchange $\\Delta S$")
