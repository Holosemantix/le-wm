#!/usr/bin/env python3
"""Draw the compact three-block Figure 1 for ACPC, IR, and SR.
...
DEFAULT_OUT = ROOT / "assets" / "paper1_figs" / "fig_acpc_ir_sr_overview.pdf"
DEFAULT_PREVIEW = ROOT / "assets" / "paper1_figs" / "fig_acpc_ir_sr_overview.png"
...
def _draw_three_token_panel(
    ...
    *,
    ...
    if robust:
        if rollout:
            # Preserve the robust within-panel geometry while making the
...
def _draw_metric_row(
    ...
    *,
    ...
    for col_x, name, trend, sub, perturbed_pair in (
        ...
        (SR_COL_X, "SR", sr_trend, sr_sub, False),
    ):
...
def plot(out_pdf: Path, preview_png: Path, input_dir: Path) -> None:
    ...
    with plt.rc_context(STYLE):
        ...
        pdf_metadata = {
            "Title": "Figure 1: three-block ACPC, IR, and SR overview",
            "Subject": (
                "Real PushT inputs, a 2-D neighborhood illustration, and "
                "pair-to-checkpoint IR/SR aggregation"
            ),
            "Creator": "paper1/scripts/plot_acpc_ir_sr_overview.py",
            "Keywords": (
                "PushT; ACPC; Invariance Radius; Separation Rate; "
