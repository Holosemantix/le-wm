...
ACPC is a radius--margin diagnostic. A visual probe of the same underlying
state should stay inside a small action-conditioned rollout tube, while states
that differ in a task-relevant way should remain outside that tube. The radius
side gives paired prediction-error and planner-cost inequalities. The margin
side prevents the low-radius solution from being a collapsed representation.
The experiments use these objects as diagnostics of frozen checkpoints, not as
...
perturbed view of the same underlying state. A frozen encoder maps them to
$z=E_\theta(h)$ and $\tilde z=E_\theta(\tilde h)$. The predictor $F_\theta$
rolls both representations forward under one shared action sequence
$\mathbf a=(a_0,\ldots,a_{H-1})$. A projection $\Pi$ selects the part of each
predicted representation used by the diagnostic, and nonnegative weights
$\alpha_k$ with $\sum_{k=1}^H\alpha_k=1$ determine how the $H$ future steps are
combined. Define
\begin{equation}
\bar G_{\mathbf a}(z)
=
...
\label{eq:acpc-h}
\end{equation}
ACPC is the radius of the same-state rollout tube: it measures how far two
visual views of one state separate after the same actions are applied. This is
different from encoder shift, which is measured before the action sequence and
therefore cannot show whether the predictor amplifies or contracts the visual
displacement. Here $H$ counts predicted future steps, not history frames, and
\emph{action matched} means that both branches receive the same recorded or
candidate actions. The stacked rollout $\bar G_{\mathbf a}$ is used for
future-error analysis; its final component is the quantity that enters the
LeWM squared-distance goal cost.

\subsection{Common-future error drift}

The first bound explains what the rollout tube can say about prediction error.
The target $Y_{\mathbf a}^H$ is not produced by either branch. It is the same
action-matched future used only for evaluation, for example the projected
latent of the actually observed future stacked and weighted like
$\bar G_{\mathbf a}$. Both branches must be compared with the same
$Y_{\mathbf a}^H$; otherwise the error difference below is not a same-future
comparison.
...
prediction is accurate. It says that, for a common action-matched future, the
visual perturbation cannot change the prediction error by more than the
paired-rollout disagreement. This is the quantity tested by the future-drift
experiment.

\subsection{Planner flips as radius--margin events}

A visual perturbation changes a CEM decision only if it moves candidate costs
enough to cross a clean candidate gap. ACPC supplies the movement side of this
comparison; the clean top-1 and elite margins supply the amount of movement
required for a flip. Consider a fixed pool of candidate action sequences
$\{\mathbf a^j\}_{j=1}^K$. Let
$x_j=\Pi(F_\theta^H(E_\theta(h),\mathbf a^j))$ and
$\tilde x_j=\Pi(F_\theta^H(E_\theta(\tilde h),\mathbf a^j))$ be the final
predicted representations for candidate $j$. Their displacement is
$r_j=\|x_j-\tilde x_j\|_2$. This is the final-step component of ACPC, and
$r_j\leq \mathrm{ACPC}_H/\sqrt{\alpha_H}$ whenever $\alpha_H>0$.
...
The quantity $b_j$ is a candidate-specific upper bound on how much the squared
goal cost can move under the visual perturbation. If the clean cost gap is
larger than the relevant pair of movement bounds, the perturbation cannot
change the winner or elite set for that fixed pool. Unlike a global Lipschitz
argument, \Cref{eq:exact-cost-bound} uses the actual paired endpoints and their
distances from the goal. It is worst-case tight from these quantities alone;
proofs and equality cases are in Appendix~\ref{sec:appendix-acpc-proofs}. The
certificate is not a compute-saving rule: it requires evaluating each candidate
under both matched histories.
...
The corollary is conditional: it ends at the first iteration where the elite
condition fails, after which later proposals may diverge. It is a shared-pool
model-cost statement, not a closed-loop return guarantee. Appendix~\ref{sec:appendix-fixed-pool-details}
...
make every same-state pair agree. We therefore distinguish nominal/probed
histories of the same state, collected in $\mathcal P_{\rm same}$, from
task-defined proxy pairs that cross a state distinction, collected in
$\mathcal P_{\rm diff}$. Let $d_\theta$ denote their normalized rollout
distance.
...
\end{equation}
\end{definition}
The first condition is the same-state tube requirement. The second condition
is the margin guard: different-state proxy pairs should stay outside that
tube. We use selective consistency only in this radius--margin sense.
Selective Margin Pass Rate (SMPR) is the empirical fraction of
different-state proxy pairs satisfying the second inequality when $r$ is the
same-state q90 tube. SMPR is designed to detect gross collapse; it does not
prove semantic validity, action faithfulness, or preservation of every
task-relevant distinction.

\subsection{Checkpoint-level diagnostic score}\label{sec:acpc-diag}

The propositions above operate on one paired history or one fixed candidate
pool. To compare entire checkpoints across tasks, we normalize each pair by a
clean transition scale and then summarize the upper tail across logged
anchors. ATR and SMPR are checkpoint-level scores: they summarize logged
anchors rather than single histories. For anchor $i$, let $s_i$ be the median
norm of its clean observed
transitions, including the history-to-future boundary:
\begin{equation}
...
\end{equation}
The numerator is the unnormalized ACPC radius used by the pairwise bounds and
planner analysis. The scale $s_i$ is an anchor-specific yardstick fixed by the
clean logged transition and is not an outcome label. ATR is the high-quantile
same-state tube radius across logged anchors; it is an offline checkpoint
summary, not an online single-history alarm and not a candidate-distribution
probability.
...
\end{equation}
The criterion $S\ge0$ means that a checkpoint meets both calibrated
...
for each model family. Given a reference checkpoint $\theta_0$, the paired
comparison is
\begin{equation}
...
The experiments use these objects in three ways. The prediction experiment
tests the common-future error-drift proposition by asking whether
recorded-action eight-step ACPC predicts the change in eight-step latent error
better than one-step ACPC or same-horizon action-destroyed controls. The
planner experiment tests the fixed-pool and adaptive-CEM quantities. The
cross-task, cross-stressor, and PLDM analyses use the checkpoint-level ATR+SMPR
score to test calibrated threshold transfer. These are separate claims:
prediction-error sensitivity, planner relevance, and calibrated checkpoint
screening are not interchangeable.
