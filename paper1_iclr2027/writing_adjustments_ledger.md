...
最近更新：2026-09-10（UTC）。各日期记录中的行号对应当次版本的 `main.tex`。
...
## 2026-09-10 全文修正作者—年份引用命令

逐处核对主文与附录引用语境：41 处 `\cite{}` 均为方法名后或陈述后的
文献依据，不是作者作为句子成分的叙述式引用，全部改为 `\citep{}`。
原有 t-SNE 的 `\citep{}` 保持不变，全文现有 42 处明确的括号式引用。
已检查附录扩展文件与表格源文件，无额外遗漏。没有需要改成 `\citet{}`
的作者叙述式语境，没有改作者、年份、引用键、引用顺序或论文措辞。

与 HEAD 对照确认 main.tex 除上述命令替换外逐字一致；bibliography 与此前
构建逐字节一致。TeX Live 2025 完整四步重建成功，已查看 PDF 第 2、3 页，
方法名后和多文献引用均正确显示外括号与作者—年份逗号。无未定义引用、
BibTeX warning 或 overfull box；主文仍 9 页、全文 25 页。已同步 main.pdf，
未提交或推送。
